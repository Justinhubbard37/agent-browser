...
Clients connect to `ws://127.0.0.1:<port>` and receive `frame`, `status`, `tabs`, `url`, and `console` messages. They send `input_mouse`, `input_keyboard`, and `input_touch` to drive the page, and `{"type":"config","maxFps":N}` (1 to 120, `0` = uncapped) to cap their own frame rate. See [streaming.md](streaming.md).
